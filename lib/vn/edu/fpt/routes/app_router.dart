...
abstract final class AppRouter {
  ...
  static Route<dynamic> onGenerateRoute(RouteSettings settings) {
    final route = switch (settings.name) {
      AppRoutes.home => _screen('Trang chủ', const HomeScreen()),
      AppRoutes.attendance => _screen('Điểm danh', const AttendanceScreen()),
      AppRoutes.timetable => _screen('Lịch học', const TimetableScreen()),
      AppRoutes.grade => _screen('Bảng điểm', const GradeScreen()),
      AppRoutes.notification => _screen(
        'Thông báo',
        const NotificationScreen(),
      ),
      AppRoutes.profile => _screen('Cá nhân', const ProfileScreen()),
      AppRoutes.login ||
  ...
  static Widget _screen(String title, Widget child) {
